...
func NewSearch(c gocube.CubieEdges, g Goal, h Heuristic,
...
func (s *Search) distSearch(st gocube.CubieEdges, max int, d int) []gocube.Move {
...
func (s *Search) regularSearch(st gocube.CubieEdges, max int) []gocube.Move {
	// If we can't search any further, check if it's the goal.
	if max == 0 {
		if s.goal.IsGoal(st) {
			return []gocube.Move{}
